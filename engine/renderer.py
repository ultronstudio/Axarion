...
class Renderer:
    ...
    # GUI SYSTEM
    def add_gui_element(self, element):
        """Add GUI element to renderer"""
        self.gui_elements.append(element)

    def remove_gui_element(self, element):
        """Remove GUI element from renderer"""
        if element in self.gui_elements:
            self.gui_elements.remove(element)

    def draw_button(self, x, y, width, height, text, font=None, 
                   bg_color=(100, 100, 100), text_color=(255, 255, 255),
                   hover_color=(150, 150, 150), border_color=(200, 200, 200)):
        ...
        pygame.draw.rect(self.screen, current_bg, (x, y, width, height))
        ...
        return is_hovered and mouse_click

    def draw_panel(self, x, y, width, height, bg_color=(50, 50, 50), 
                  border_color=(100, 100, 100), alpha=255):
        """Draw a GUI panel"""
        panel_surface = pygame.Surface((width, height))
        panel_surface.set_alpha(alpha)
        panel_surface.fill(bg_color)
        pygame.draw.rect(panel_surface, border_color, (0, 0, width, height), 2)
        self.screen.blit(panel_surface, (x, y))

    def draw_slider(self, x, y, width, value, min_val=0, max_val=100,
                   bg_color=(100, 100, 100), slider_color=(200, 200, 200)):
        """Draw a slider and return new value if dragged"""
        height = 20
        mouse_pos = pygame.mouse.get_pos()
        mouse_click = pygame.mouse.get_pressed()[0]
        ...
        # Draw slider handle
        pygame.draw.circle(self.screen, slider_color, 
                          (int(slider_pos), y + height // 2), height // 2)

        # Handle dragging
        if (mouse_click and x <= mouse_pos[0] <= x + width and 
            y <= mouse_pos[1] <= y + height):
    ...
    def draw_menu(self, x, y, items, selected_index=0, 
                 bg_color=(70, 70, 70), selected_color=(120, 120, 120),
                 text_color=(255, 255, 255)):
        """Draw a menu and return selected index"""
        mouse_pos = pygame.mouse.get_pos()
        mouse_click = pygame.mouse.get_pressed()[0]
        ...
        for i, item in enumerate(items):
            ...
            is_hovered = (x <= mouse_pos[0] <= x + menu_width and 
                         item_y <= mouse_pos[1] <= item_y + item_height)

            # Choose background color
            current_bg = selected_color if (i == selected_index or is_hovered) else bg_color

            # Draw item background
            pygame.draw.rect(self.screen, current_bg, 
                           (x, item_y, menu_width, item_height))

            # Draw item text
            text_surface = self.default_font.render(item, True, text_color)
            self.screen.blit(text_surface, (x + 10, item_y + 5))

            # Return clicked item index
            if is_hovered and mouse_click:
                return i
